...
impl CompressedRistretto {
    /// Convert this `CompressedRistretto` to an array of bytes.
    ...
    pub fn decompress(&self) -> Option<RistrettoPoint> {
        ...
        let s_encoding_is_canonical =
            subtle::arrays_equal(&s_bytes_check[..], self.as_bytes());
...
impl CTAssignable for RistrettoPoint {
...
pub mod vartime {
    ...
    pub fn k_fold_scalar_mult<'a, 'b, I, J>(scalars: I, points: J) -> RistrettoPoint
        where I: IntoIterator<Item = &'a Scalar>,
              J: IntoIterator<Item = &'b RistrettoPoint>
    {
        let extended_points = points.into_iter().map(|P| &P.0);
        RistrettoPoint(curve::vartime::k_fold_scalar_mult(scalars, extended_points))
    }
}
...
mod test {
    ...
    fn scalarmult_ristrettopoint_works_both_ways() {
        let P: RistrettoPoint = RistrettoPoint(constants::ED25519_BASEPOINT);
        let s: Scalar = Scalar::from_u64(999);
